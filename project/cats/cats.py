...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    picked_list = list(filter(select,paragraphs))
    return str(picked_list[k]) if len(picked_list) >= k+1 else ''
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    if not source_words and not typed_words:
        ...
        count = [typed_words[i] == source_words[i] for i in range(length)]
        return count.count(True)/len(typed_words)*100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed,source) in cache:
            value, limit_cached  = cache[(typed,source)]
            # print("DEBUG:","find!!",value,limit_cached)
            if limit <=l imit_cached:
                return value
            else:
            ...
            # print("DEBUG:","not find ,recalculating")
            value = diff_function(typed, source, limit)
            cache[(typed,source)] = (value,limit)
            return value
...
def furry_fixes(typed, source, limit):
    ...
    #     return (0 if (typed[0] == source[0]) else 1) + abs(len(typed) - len(source))
    # if typed[0] == source[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    #     return 1
    # if len(typed) == 1:
    #     return (0 if (typed in source) else 1) + abs(len(typed) - len(source))
    # if len(source) ==1:
    #     return (0 if (source in typed) else 1) + abs(len(source) - len(typed))
    ...
    if typed[0] == source[0]:
        return minimum_mewtations(typed[1:], source[1:], limit)
    return 1 + min(
        minimum_mewtations(typed, source[1:], limit-1),    # insert
...
def report_progress(typed, source, user_id, upload):
    ...
    for index,word in enumerate(typed):
        if word== source[index]:
            ...
            break
    dic = {'id': user_id, 'progress': cnt/len(source)}
    upload(dic)
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[t_user[i+1]-t_user[i] for i in range(len(t_user)-1)] for t_user in timestamps_per_player]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    #     lst = [times[user][i] for user in player_indices]
    #     # print("DEBUG:",lst)
    #     # print("DEBUG:",lst.index(min(lst)))
    #     # print("DEBUG:",result[lst.index(min(lst))])
    #     result[lst.index(min(lst))].append(words[i])
    # return result
    # # END PROBLEM 10


    words, times = words_and_times.values()
    result = [[] for _ in times]
    for i, word in enumerate(words):
        fastest = min(range(len(times)), key=lambda p: times[p][i])
        result[fastest].append(word)
    return result

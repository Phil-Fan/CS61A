def divide(quotients, divisors):
    ...
    """
    return {x: list(filter(lambda m: m % x == 0, divisors)) for x in quotients}
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1,amount//price+1):
                ...
                add(fruits[1:] if len(fruits)>1 else [], amount-k*price, cart+display(fruit,k))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(point,city_a) < distance(point,city_b) else get_name(city_b)
